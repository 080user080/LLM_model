...
# повністю заміни функцію:
def is_cap_name_like(token: str) -> bool:
    if token in STOP_CAP:
        return False
    parts = token.split()
    if parts and parts[0].lower() in DET_FIRST:
        return False
    low = token.lower()
    if len(parts) == 1 and (low in STOP_ANY_LOWER or len(token) <= 2):
        return False
    for p in parts:
        if not re.match(rf"^[{UKR_UP}]", p):
            return False
        if p.lower() in STOP_ANY_LOWER:
            return False
