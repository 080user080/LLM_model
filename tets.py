...
def ensure_yaml_file(path: str = DEFAULT_YAML_FILENAME) -> dict:
    ...
    try:
        ...
        for section in ("structural", "actions"):
            for itm in parsed.get(section, []):
                ...
                if isinstance(fstr, str):
                    ...
                    flags = fstr
                # захист: якщо pattern вже рядок - компілюємо, якщо вже regex - використовуємо
                if isinstance(pattern, str):
                    compiled = re.compile(pattern, flags)
                else:
                    # очікуємо, що PyYAML дав рядок; але на всякий випадок
        ...
        # PyYAML відсутній або парсинг провалився - повертаємо вбудований словник
    ...
    for chunk in paragraphs:
        ...
        for rule in SFX_RULES:
            ...
            for m in rule["keywords_re"].finditer(para):
                start = m.start()
                # знайдемо позицію першої пунктуації після m.end() у межах абзацу
                rest = para[m.end():]
                ...
                if punct_match:
                    ...
                    # немає пунктуації в межах абзацу -> беремо кінець фрагменту (до наступного newline або до кінця)
                    nl = re.search(r'\n', rest)
                    if nl:
                        end = m.end() + nl.start()
                    else:
        ...
        # Вставка: працюємо зі строкою, накопичуємо частини
        out_parts = []
        idx = 0
        for s in final_spans:
            # end вже включає знак пунктуації якщо він існує, отже тег буде після крапки.
